...
svd_results = pd.read_csv(os.path.join('results', 'svd_results.csv'))      # get optimal number of SVD components per store
...
ar_order = 1                   # default AR order to 1
ma_order = 1                   # default MA order to 1
...
for store_num in store_sales.Store.unique():
    ...
    # evaluate model for different combinations of seasonal AR and seasonal MA orders
    for sar_order in tqdm(range(0, 4)):
        for sma_order in range(0, 4):
            # train SARIMA+SVD model and make sales forecast for the next 52 weeks
            model = SARIMAX_Model(p=ar_order, q=ma_order, P=sar_order, Q=sma_order, diff_order=diff_order, 
                                  transform=transform_method, model_type='SARIMA')
            ...
            sar_list.append(sar_order)
            sma_list.append(sma_order)
...
metrics = pd.DataFrame({'Store': store_list, 
                        ...
                        'Seasonal AR Order': sar_list, 'Seasonal MA Order': sma_list,
...
metrics['Model'] = ["SARIMA"] * metrics.shape[0]
metrics.to_csv(os.path.join('results', 'sarima_metrics.csv'), index=False)